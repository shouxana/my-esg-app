    ...
    const query = `
      ...
      age_categories AS (
          SELECT
              ...
              year,
              CASE
                  WHEN is_manager AND EXTRACT(YEAR FROM AGE(MAKE_DATE(year, 12, 31), birth_date)) < 30 
                      THEN 'Managers Under 30'
                  WHEN is_manager AND EXTRACT(YEAR FROM AGE(MAKE_DATE(year, 12, 31), birth_date)) BETWEEN 30 AND 49 
                      THEN 'Managers 30-50'
                  WHEN is_manager AND EXTRACT(YEAR FROM AGE(MAKE_DATE(year, 12, 31), birth_date)) >= 50 
                      THEN 'Managers Over 50'
                  WHEN EXTRACT(YEAR FROM AGE(MAKE_DATE(year, 12, 31), birth_date)) < 30 
                      THEN 'Under 30'
                  WHEN EXTRACT(YEAR FROM AGE(MAKE_DATE(year, 12, 31), birth_date)) BETWEEN 30 AND 49 
                      THEN '30-50'
                  ELSE 'Over 50'
              END as age_category
          FROM employee_base
