...
export async function GET(request: Request) {
  ...
  WITH employee_leaves AS (
    SELECT
      ...
      END as status,
      TO_CHAR(e.leave_date_start, 'YYYY-MM-DD') as leave_date_start,
      CASE 
        WHEN e.leave_date_end IS NULL THEN NULL
        ELSE TO_CHAR(e.leave_date_end, 'YYYY-MM-DD')
      END as leave_date_end,
      CASE 
        WHEN e.leave_date_end IS NULL THEN
          (CURRENT_DATE - e.leave_date_start::date)
        ELSE
          (e.leave_date_end::date - e.leave_date_start::date)
    ...
    WHERE 
      ...
      AND g.gender = $2
      AND EXTRACT(YEAR FROM e.leave_date_start::date) = $3::integer
